...
export const getGroupsByTable = asyncHandler(async (req: Request, res: Response) => {
  const { table: tableId } = req.query;
  if (!tableId) return sendError(res, 'Table ID is required');
  ...
  const groupsWithStats = await Promise.all(
    groups.map(async (g) => {
      const orders = await Order.find({ group: g._id, paymentStatus: 'unpaid' });
      return {
        ...g.toJSON(),
...
export const createGroup = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!tableId || !guestCount) return sendError(res, 'tableId and guestCount are required');
  ...
  const activeGroups = await Group.find({ table: tableId, paymentStatus: 'unpaid' });
...
export const getGroupBill = asyncHandler(async (req: Request, res: Response) => {
  const group = await Group.findById(req.params.id).populate<{ table: ITable }>('table');
  ...
  const orders = await Order.find({ group: req.params.id, paymentStatus: 'unpaid' });
...
export const payGroup = asyncHandler(async (req: Request, res: Response) => {
  ...
  const group = await Group.findById(req.params.id).populate<{ table: ITable }>('table');
  if (!group) return sendError(res, 'Group not found', 404);
  if (group.paymentStatus === 'paid') return sendError(res, 'Group has already paid', 400);
