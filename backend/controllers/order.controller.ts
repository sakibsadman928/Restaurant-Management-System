...
export const createOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  const group = await Group.findById(groupId).populate<{ table: ITable }>('table');
  if (!group) return sendError(res, 'Group not found', 404);
  if (group.paymentStatus === 'paid') return sendError(res, 'This group has already paid', 400);
  ...
  if (items && items.length > 0) {
    for (const item of items) {
      ...
      if (!menuItem) return sendError(res, `Menu item not found: ${item.menuItemId}`, 404);
...
export const getOrders = asyncHandler(async (req: Request, res: Response) => {
  const { status, table, group } = req.query;
  const filter: Record<string, any> = {};

  if (req.user!.role === 'waiter') {
    if (table) {
      const owns = await Table.exists({ _id: table, assignedWaiter: req.user!.id });
      if (!owns) return sendError(res, 'You are not assigned to this table', 403);
      filter.table = table;
    } else if (group) {
      filter.group = group;
    } else {
      const tables = await Table.find({ assignedWaiter: req.user!.id }).select('_id');
...
export const updateItems = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (order.status !== 'pending')
    return sendError(res, 'Order cannot be modified after being sent to kitchen', 400);
...
export const updateStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  const order = await Order.findById(req.params.id).populate<{ table: ITable }>('table');
  if (!order) return sendError(res, 'Order not found', 404);
  if (order.status !== 'preparing')
    return sendError(res, 'Only preparing orders can be updated by the kitchen', 400);
